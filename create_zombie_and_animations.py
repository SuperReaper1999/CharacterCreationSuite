# create_zombie_and_animations.py
#
# Zombie variant of create_character_and_animations.py. Same rig, mesh
# build, and Idle/Run/Jump logic as the player character - this file does
# NOT modify create_character_and_animations.py in any way, it's a
# separate copy so the working player pipeline stays untouched.
#
# Differences from the player version:
#   - Shirt material is red instead of blue.
#   - Object/armature/mesh names are prefixed "Zombie_" instead of
#     "BoxHumanoid_" so importing both characters into the same Unity
#     project doesn't collide on asset/object names.
#   - Output filenames are zombie_rigged.glb / animated_zombie_rigged.fbx.
#   - Adds a fourth Action: Attack (frames 1-30, no loop) - a wind-up /
#     forward swipe / recovery pose sequence, same style (local X-axis
#     bone rotations only, no translation) as Idle/Run/Jump.
#
# Run:
#   python create_zombie_and_animations.py
...
def run_launcher():
    ...
    static_output = os.path.join(current_folder, "zombie_rigged.glb")
    animated_output = os.path.join(current_folder, "animated_zombie_rigged.fbx")

    print("Zombie character pipeline")
    print("--------------------------")
    ...
    print()
    print("=== Building zombie box humanoid (static GLB + animated FBX) ===")
    ...
    for path, label in (
        (static_output, "zombie_rigged.glb"),
        (animated_output, "animated_zombie_rigged.fbx"),
    ):
    ...
    print()
    print("Takes in the FBX: Idle (loop), Run (loop), Jump (no loop), Attack (no loop)")
    print("These should import as four separate clips - no manual splitting.")
...
def run_build():
    ...
    shirt_mat = create_material("Red Shirt", (0.65, 0.06, 0.06, 1.0))
    ...
    # -----------------------------
    # Same fix as create_character_and_animations.py (this file's geometry is
    # a copy of it) - authored height 2.69m scaled uniformly about the origin
    # (feet stay planted at z=0) down to a realistic average height, so Unity
    # import needs no manual scale correction afterward.
    ...
    armature_obj.name = "Zombie_Armature"
    armature_data = armature_obj.data
    armature_data.name = "ZombieSkeleton"
    ...
    bpy.ops.export_scene.gltf(
        filepath="zombie_rigged.glb",
        export_format="GLB",
        use_selection=True,
    )

    print("Exported zombie_rigged.glb (bind pose)")
    ...
    # -----------------------------
    # Attack: frames 1-30 (no loop)
    # Wind-up (arms raised back) -> swipe (arms swing forward/down hard,
    # chest lunges forward) -> recovery -> neutral. Same style as the
    # other clips: local X-axis bone rotations only, no translation.
    # -----------------------------

    attack_action = new_action("Attack", 1, 30)
    scene.frame_start, scene.frame_end = 1, 30

    attack_poses = {
        1: {},  # neutral
        8: {  # wind-up: arms raised back, chest leans back slightly
            "UpperArm.L": (math.radians(-70), 0, 0),
            "UpperArm.R": (math.radians(-70), 0, 0),
            "Forearm.L": (math.radians(-20), 0, 0),
            "Forearm.R": (math.radians(-20), 0, 0),
            "Chest": (math.radians(-8), 0, 0),
        },
        16: {  # strike: arms swing forward/down hard, chest lunges forward
            "UpperArm.L": (math.radians(80), 0, 0),
            "UpperArm.R": (math.radians(80), 0, 0),
            "Forearm.L": (math.radians(25), 0, 0),
            "Forearm.R": (math.radians(25), 0, 0),
            "Chest": (math.radians(15), 0, 0),
        },
        24: {  # recovery
            "UpperArm.L": (math.radians(30), 0, 0),
            "UpperArm.R": (math.radians(30), 0, 0),
            "Chest": (math.radians(5), 0, 0),
        },
        30: {},  # back to neutral
    }

    for frame, pose in attack_poses.items():
        scene.frame_set(frame)
        reset_pose()
        for bone_name, rot in pose.items():
            key_bone(bone_name, frame, rot)
    ...
    set_interpolation(attack_action)

    # -----------------------------
    # Export animated FBX - all four actions as separate Takes
    # -----------------------------
    ...
    bpy.ops.export_scene.fbx(
        filepath="animated_zombie_rigged.fbx",
        use_selection=True,
    ...
    print("Exported animated_zombie_rigged.fbx")
    print("Takes: Idle (1-60, loop), Run (1-31, loop), Jump (1-46, no loop), Attack (1-30, no loop)")
